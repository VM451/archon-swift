...
public final class OpenAIProvider: EmbeddingProvider, LLMProvider, @unchecked Sendable {
    ...
    public func embed(text: String) async throws -> [Float] {
        ...
        guard let first = batchResult.first else {
            throw NSError(domain: "ArchonMemory.OpenAIProvider", code: 1, userInfo: [NSLocalizedDescriptionKey: "Empty embedding response"])
        }
        return first
    }
    ...
    public func embed(batch: [String]) async throws -> [[Float]] {
        ...
        guard let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) else {
            throw NSError(domain: "ArchonMemory.OpenAIProvider", code: 2, userInfo: [NSLocalizedDescriptionKey: "OpenAI Embeddings HTTP error"])
        }

        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let dataArray = json["data"] as? [[String: Any]]
        else {
            throw NSError(domain: "ArchonMemory.OpenAIProvider", code: 3, userInfo: [NSLocalizedDescriptionKey: "Invalid OpenAI embeddings JSON format"])
        }
    ...
    public func generateStructuredOutput<T: Decodable & Sendable>(
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) else {
            throw NSError(domain: "ArchonMemory.OpenAIProvider", code: 4, userInfo: [NSLocalizedDescriptionKey: "OpenAI Chat HTTP error"])
        }
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw NSError(domain: "ArchonMemory.OpenAIProvider", code: 5, userInfo: [NSLocalizedDescriptionKey: "Invalid OpenAI chat completion JSON response"])
        }
