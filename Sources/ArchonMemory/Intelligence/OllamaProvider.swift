...
public final class OllamaProvider: EmbeddingProvider, LLMProvider, @unchecked Sendable {
    ...
    public func embed(text: String) async throws -> [Float] {
        ...
        guard let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) else {
            throw NSError(domain: "ArchonMemory.OllamaProvider", code: 1, userInfo: [NSLocalizedDescriptionKey: "Ollama embedding HTTP error"])
        }

        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
              let embeddingDoubles = json["embedding"] as? [Double]
        else {
            throw NSError(domain: "ArchonMemory.OllamaProvider", code: 2, userInfo: [NSLocalizedDescriptionKey: "Invalid Ollama embedding JSON"])
        }
    ...
    public func generateStructuredOutput<T: Decodable & Sendable>(
        ...
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let httpResponse = response as? HTTPURLResponse, (200...299).contains(httpResponse.statusCode) else {
            throw NSError(domain: "ArchonMemory.OllamaProvider", code: 3, userInfo: [NSLocalizedDescriptionKey: "Ollama chat completion HTTP error"])
        }
        ...
        guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
            ...
            throw NSError(domain: "ArchonMemory.OllamaProvider", code: 4, userInfo: [NSLocalizedDescriptionKey: "Invalid Ollama chat response"])
        }
