...
public final class ModelLibraryViewModel: ObservableObject {
    ...
    /// Latest attempt snapshot per variant (try count, resume offset, delta
    /// reuse). Raw detail for diagnostics; surface through
    /// `attemptDisplayError(variantID:)` when showing errors.
    @Published public private(set) var attempts: [String: ModelDownloadAttempt] = [:]
    ...
    /// Records the latest attempt snapshot for a variant.
    public func recordAttempt(variantID: String, attempt: ModelDownloadAttempt) {
        attempts[variantID] = attempt
    }

    /// Redacted attempt failure for SwiftUI surfaces, or nil when the latest
    /// attempt carries no error.
    public func attemptDisplayError(variantID: String) -> String? {
        guard let lastError = attempts[variantID]?.lastError, !lastError.isEmpty else { return nil }
        return Self.redactedText(lastError)
    }
    ...
    public static func redactedMessage(for error: Error) -> String {
        ...
        return redactedText(error.localizedDescription)
    }

    /// Redacts raw failure text (download errors, attempt errors) with the
    /// same path/URL/credential rules used for thrown errors.
    public static func redactedText(_ raw: String) -> String {
        var redacted = raw
        ...
        do {
            ...
            for try await event in events {
                ...
                case .downloading(let value, _, _, let attempt):
                    ...
                    if let attempt { recordAttempt(variantID: event.variantID, attempt: attempt) }
                case .paused(let attempt):
                    if let attempt { recordAttempt(variantID: event.variantID, attempt: attempt) }
                case .ready:
                    progress.removeValue(forKey: event.variantID)
                    attempts.removeValue(forKey: event.variantID)
                    await refresh()
                case .failed(let message, let attempt):
                    if let attempt { recordAttempt(variantID: event.variantID, attempt: attempt) }
                    lastError = message
                    userVisibleError = Self.redactedText(message)
                    state = .failed(message)
                case .cancelled:
                    progress.removeValue(forKey: event.variantID)
                    attempts.removeValue(forKey: event.variantID)
