...
struct CloudKitSyncTests {
    ...
    func testCKRecordRoundtrip() {
        ...
        let zoneID = CKRecordZone.ID(zoneName: "ArchonPrivateZone", ownerName: CKCurrentUserDefaultName)
    ...
    func testConflictResolutionVersion() {
        let engine = CloudKitSyncEngine(containerId: "iCloud.com.archon.test")
        let now = Date()
